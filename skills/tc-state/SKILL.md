---
name: tc-state
description: TC 创业状态存档工具。用户说“/tc-state”“保存这次创业判断”“记住当前方案”“恢复上次 TC 进度”“继续上次创业问题”“列出 TC 存档”“生成阶段报告”或明确要求把已脱敏市场证据交给 TC 知识候选时使用。把确认的问题、判断、行动和证据保存到本机，或在逐次授权后导出本机候选；不自动上传，不替用户保存敏感原文。
---
...
只有用户明确说“把这次结果作为知识候选”时才执行：

1. 先说明只生成本机文件，不上传；
2. 使用 `export-evidence --consent` 导出；
3. 检查导出内容，确认没有姓名、联系方式、客户、链接、账号、合同和商业机密；
4. 再交给 `tc-knowledge candidate-add` 保存为本机 L1 候选。

导出默认只保留：问题定义、当前判断、下一步、有效标准和用户主动记录的证据。脚本会移除常见邮箱、手机号、链接和凭据格式，但自动脱敏不能替代人工复核。

一次状态或一个成功案例不能升级成稳定方法。只有多个独立案例出现相近场景、动作和结果，并通过许可、隐私、反例与副作用检查，才允许由维护者人工提炼。
